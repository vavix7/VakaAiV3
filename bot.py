...
from config import (
    ...
    SUMMON_COOLDOWN_SECONDS, WELCOME_ENABLED, RULES_TITLE, AI_ENABLED, AI_API_KEY, AI_BASE_URL, AI_MODEL, RANKS, BACKUP_CHAT_ID, BACKUP_INTERVAL_HOURS, AI_FALLBACK_MODELS, AI_GEMINI_API_KEY, AI_GROQ_API_KEY, AI_MISTRAL_API_KEY, AI_OPENROUTER_API_KEY, AI_CLOUDFLARE_API_TOKEN, AI_CLOUDFLARE_ACCOUNT_ID, CLEANUP_CHAT_ID, CLEANUP_INTERVAL_HOURS, GOOGLE_DRIVE_BACKUP_ENABLED, GOOGLE_DRIVE_CREDENTIALS_FILE, GOOGLE_DRIVE_FOLDER_ID, GOOGLE_DRIVE_FOLDER_NAME, GOOGLE_DRIVE_BACKUP_COUNT, EMERGENCY_BACKUP_ENABLED, LOCAL_BACKUP_COUNT, LOG_CLEANUP_ENABLED, LOG_RETENTION_DAYS, TRAINING_ENABLED, TRAINING_DAY, TRAINING_TIME, TRAINING_TEAM_SIZE, AUTO_MUTE_WARNINGS, AUTO_BAN_WARNINGS, REPORTS_ENABLED, SHOP_ENABLED, SHOP_DONATE_APPROVAL, SHOP_BOOST_APPROVAL, DATABASE_CHAT_ID
)
...
try:
    marker = db.conn.execute("SELECT 1 FROM migration_meta WHERE key='v29_admin_cleanup'").fetchone()
    if not marker:
        db.conn.execute("DELETE FROM admin_roles WHERE telegram_id<>?", (int(OWNER_ID),))
        db.set_admin_role(int(OWNER_ID),8)
        db.conn.execute("INSERT OR REPLACE INTO migration_meta(key,value) VALUES('v29_admin_cleanup','1')")
        db.conn.commit()
except Exception:
    logger.exception("Не удалось очистить старые назначения администраторов")
...
gdrive = GoogleDriveBackup(GOOGLE_DRIVE_CREDENTIALS_FILE, GOOGLE_DRIVE_FOLDER_ID, GOOGLE_DRIVE_FOLDER_NAME)
...
def create_backup() -> Path:
    ...
    for old_file in backup_files()[LOCAL_BACKUP_COUNT:]:
        try:
            old_file.unlink()
        except OSError:
            logger.warning("Не удалось удалить старый бэкап %s", old_file)
    if GOOGLE_DRIVE_BACKUP_ENABLED:
        try: gdrive.upload(target)
        except Exception: logger.exception("Google Drive upload failed")
    return target
...
def backup_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="☁️ Google Drive", callback_data="owner_drive_backups")],
...
def is_admin(user_id: int, minimum_rank: int = 6) -> bool:
    """Dynamic hierarchy check. Rank 8 is immutable; lower ranks are controlled by the Leader."""
    return get_admin_rank(int(user_id)) >= int(minimum_rank)
...
def admin_keyboard(user_id: int | None = None):
    """Главное меню администрации. Разделы показываются по матрице доступа."""
    uid = int(user_id or 0)
    rank = get_admin_rank(uid)
    if rank < 1:
        return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="menu_main")]])

    def allowed(key):
        return rank >= 8 or db.has_permission(rank, key)

    rows=[]
    if allowed("players"):
        rows.append([InlineKeyboardButton(text="👥 Управление игроками", callback_data="admin_section_players")])
    if allowed("activity"):
        rows.append([InlineKeyboardButton(text="🔥 Активность", callback_data="admin_section_activity")])
    if allowed("kv"):
        rows.append([InlineKeyboardButton(text="⚔️ КВ", callback_data="admin_section_kv")])
    if allowed("tournaments"):
        rows.append([InlineKeyboardButton(text="🏆 Турниры", callback_data="admin_section_tournaments")])
    if allowed("moderation") or allowed("ranks") or allowed("reports") or allowed("logs"):
        rows.append([InlineKeyboardButton(text="🛡 Администрация", callback_data="admin_section_administration")])
    if rank >= 8:
        rows.append([InlineKeyboardButton(text="🔐 Настройки доступа", callback_data="admin_access_settings")])
    if allowed("training"):
        rows.append([InlineKeyboardButton(text="🏋️ Тренировки", callback_data="admin_training")])
    if allowed("shop"):
        rows.append([InlineKeyboardButton(text="🛒 Настройка магазина", callback_data="admin_shop_settings")])
    if rank >= 8:
        rows.append([InlineKeyboardButton(text="💾 Архив и данные", callback_data="owner_backups")])
        rows.append([InlineKeyboardButton(text="🪙 Коины", callback_data="admin_section_coins")])
...
async def callback_admin_section_players(callback: CallbackQuery):
    if not permission(callback.from_user.id,"players"):
...
async def callback_admin_section_activity(callback: CallbackQuery):
    if not permission(callback.from_user.id,"activity"):
        await callback.answer("🔒 Только руководство (6–8).", show_alert=True); return
...
async def callback_admin_section_kv(callback: CallbackQuery):
    if not permission(callback.from_user.id,"kv"):
        await callback.answer("🔒 Только руководство (6–8).", show_alert=True); return
...
async def callback_admin_section_administration(callback: CallbackQuery):
    if not any(permission(callback.from_user.id,k) for k in ("moderation","ranks","reports","logs")):
        await callback.answer("🔒 Только руководство (6–8).", show_alert=True); return
...
async def callback_admin_section_tournaments(callback: CallbackQuery):
    if not permission(callback.from_user.id,"tournaments"):
        await callback.answer("🔒 Только руководство (6–8).", show_alert=True); return
    await callback.answer()
...
async def callback_admin_section_ranks(callback: CallbackQuery):
    if not permission(callback.from_user.id,"ranks"):
        await callback.answer("🔒 Только руководство (6–8).", show_alert=True); return
    await callback.answer()
...
async def callback_menu_profile(callback: CallbackQuery):
    ...
    await safe_edit(callback.message, enhanced_player_card(player["player_id"]), reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⚠️ История нарушений", callback_data=f"profile_history_{player['telegram_id']}")],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="menu_main")]
    ]))
    await callback.answer()
...
async def callback_menu_admin(callback: CallbackQuery):
    if get_admin_rank(callback.from_user.id)<1 or (get_admin_rank(callback.from_user.id)<8 and 'ACCESS_KEYS' in globals() and not any(db.has_permission(get_admin_rank(callback.from_user.id),k) for k in ACCESS_KEYS)):
        await callback.answer("🔒 Только для администратора.", show_alert=True)
        return
...
def shop_keyboard():
    cats=db.get_shop_categories()
    if not cats:
        return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Прочее",callback_data="shop_cat_extra")]])
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"📂 {c['name']}",callback_data=f"shop_cat_db_{c['category_id']}")] for c in cats])

def shop_category_keyboard(prefix):
    if prefix.startswith("db_"):
        cid=int(prefix.split("_",1)[1]); rows=[[InlineKeyboardButton(text=f"📦 {r['name']} — {r['price']} 🪙",callback_data=f"shop_product_{r['product_id']}")] for r in db.conn.execute("SELECT * FROM shop_products WHERE active=1 AND category_id=? ORDER BY product_id",(cid,)).fetchall()]
        rows.append([InlineKeyboardButton(text="⬅️ Магазин",callback_data="menu_shop")]); return InlineKeyboardMarkup(inline_keyboard=rows)
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"🛍 {r['name']} — {r['price']} 🪙",callback_data=f"shop_product_{r['product_id']}")] for r in db.get_shop_products()]+[[InlineKeyboardButton(text="⬅️ Магазин",callback_data="menu_shop")]])

def shop_text(prefix):
    if prefix.startswith("db_"):
        cid=int(prefix.split("_",1)[1]); c=db.conn.execute("SELECT * FROM shop_categories WHERE category_id=?",(cid,)).fetchone(); return f"🛒 <b>{html.escape(c['name']) if c else 'Каталог'}</b>\n\n{html.escape(c['description'] or '') if c else ''}"
    return "🛒 <b>МАГАЗИН</b>\n\nВыбери товар."
...
async def callback_shop_category(callback: CallbackQuery):
    ...
    if category not in {"chat","donate","boost","extra"} and not category.startswith("db_"):
...
async def callback_shop_buy(callback: CallbackQuery):
    ...
    if key.startswith("product_"):
        try: pid=int(key.split('_',1)[1])
        except Exception: await callback.answer("Товар не найден",show_alert=True); return
        ...
        await callback.answer(f"Заявка #{rid} создана"); await callback.message.answer(f"📨 Заявка <b>#{rid}</b> создана: {html.escape(row['name'])}. После одобрения администрации коины будут списаны."); return
...
async def callback_shop_extra_product(callback: CallbackQuery):
    ...
    product_rows = db.get_shop_products(False) if permission(callback.from_user.id,"shop") else db.get_shop_products(True)
    row=next((r for r in product_rows if int(r["product_id"])==pid),None)
    if not row: await callback.answer("Товар не найден",show_alert=True); return
    text=f"🛒 <b>{html.escape(row['name'])}</b>\n\n{html.escape(row['description'] or 'Описание отсутствует.')}\n\n💰 Цена: <b>{int(row['price'])} 🪙</b>"
    buttons=[[InlineKeyboardButton(text="🛒 Купить",callback_data=f"shop_buy_product_{pid}")]]
    if permission(callback.from_user.id,"shop"):
        buttons.append([InlineKeyboardButton(text="✏️ Редактировать",callback_data=f"shop_edit_{pid}"),
                          InlineKeyboardButton(text="👁 Показать" if not int(row["active"]) else "🙈 Скрыть",callback_data=f"shop_hide_{pid}")])
        buttons.append([InlineKeyboardButton(text="📦 К товарам",callback_data="shop_admin_products")])
    buttons.append([InlineKeyboardButton(text="⬅️ Назад",callback_data="menu_shop")])
    kb=InlineKeyboardMarkup(inline_keyboard=buttons)
    try:
        if row["photo_file_id"]:
            await callback.message.delete(); await callback.message.answer_photo(row["photo_file_id"],caption=text,reply_markup=kb)
        else: await safe_edit(callback.message,text,reply_markup=kb)
    except Exception: await safe_edit(callback.message,text,reply_markup=kb)
    await callback.answer()
...
async def callback_shop_approve(callback: CallbackQuery):
    if not permission(callback.from_user.id,"shop_approve"): await callback.answer("Нет права одобрять покупки.",show_alert=True); return
...
async def command_shopadd(message: Message):
    ...
    parts=(message.text or "").split(maxsplit=2)
    if len(parts)<3 or not parts[1].isdigit(): await message.answer("Формат: /shopadd ЦЕНА НАЗВАНИЕ | ОПИСАНИЕ | ID_КАТЕГОРИИ"); return
    price=int(parts[1]); vals=parts[2].split('|'); name=vals[0].strip(); desc=vals[1].strip() if len(vals)>1 else ''; cat=int(vals[2].strip()) if len(vals)>2 and vals[2].strip().isdigit() else None
    if price<=0 or not name: await message.answer("❌ Некорректная цена/название."); return
    pid=db.add_shop_product(name,price,desc); db.update_shop_product(pid,category_id=cat); await message.answer(f"✅ Товар добавлен. ID: <code>{pid}</code>")
...
async def v29_join_request(event):
    if int(event.chat.id)!=int(GUILD_CHAT_ID): return
    uid=int(event.from_user.id); player=db.get_player_by_telegram(uid); banned=bool(player and int(player['is_banned'] or 0))
    try:
        ban=db.get_ban(GUILD_CHAT_ID,uid); banned=banned or bool(ban and int(ban['active']))
    except Exception: pass
    try:
        if player and not banned:
            await bot.approve_chat_join_request(event.chat.id,uid); db.log('auto_join_approved',uid,{'chat_id':event.chat.id})
        else:
            await bot.decline_chat_join_request(event.chat.id,uid); db.log('auto_join_declined',uid,{'chat_id':event.chat.id,'registered':bool(player),'banned':banned})
    except Exception: logger.exception('Ошибка автообработки заявки на вступление')
...
async def _ai_openai_compatible(session, base_url, api_key, model, question, provider_name):
    ...
    payload = {
        "model": model,
        "messages": [
            {"role": "system", "content": "Ты Вака — дружелюбный помощник гильдии Free Fire. Отвечай кратко, по делу, на русском языке."},
...
async def _ai_gemini(session, model, api_key, question):
    ...
    payload = {"contents": [{"parts": [{"text": "Ты Вака — дружелюбный помощник гильдии Free Fire. Отвечай кратко, по делу, на русском языке.\n\n" + question}]}], "generationConfig": {"temperature": 0.7}}
...
# =========================================================
# V2.9 — UNIFIED ACCESS / MODERATION / SHOP / TRAINING / REPORTS
# =========================================================

ACCESS_KEYS = {
    "players":"👥 Игроки", "moderation":"🛡 Модерация", "kv":"⚔️ КВ", "tournaments":"🏆 Турниры",
    "ranks":"👑 Ранги", "backups":"💾 Бэкапы", "shop":"🛒 Магазин", "shop_approve":"💎 Одобрение магазина",
    "training":"🏋️ Тренировки", "reports":"📩 Жалобы", "logs":"📜 Журнал", "activity":"🔥 Активность"
}

def _seed_v29_access():
    defaults={}
    for role in range(1,8):
        defaults[role]={k:(role>=6 if k in ("moderation","logs") else role>=7 if k in ("ranks","backups","shop","shop_approve","training","reports") else role>=3) for k in ACCESS_KEYS}
    defaults[8]={k:True for k in ACCESS_KEYS}
    db.seed_access_permissions(defaults)
_seed_v29_access()

def _seed_v29_shop_catalog():
    cats=db.get_shop_categories(False); names={c['name']:c['category_id'] for c in cats}
    for name in ('Чат','Донат','БУСТ'):
        if name not in names: names[name]=db.add_shop_category(name)
    for key,name,price,desc in SHOP_CORE_PRODUCTS:
        if db.conn.execute('SELECT 1 FROM shop_products WHERE action_key=?',(key,)).fetchone(): continue
        cat='Чат' if key in {'all_locks','unban','unmute','unwarn'} else 'Донат' if key in {'lite','classic','pro'} else 'БУСТ'
        pid=db.add_shop_product(name,price,desc); db.update_shop_product(pid,category_id=names[cat],action_key=key)
_seed_v29_shop_catalog()
try: db.set_training_settings(enabled=TRAINING_ENABLED,weekday=TRAINING_DAY,start_time=TRAINING_TIME,team_size=TRAINING_TEAM_SIZE)
except Exception: pass

def permission(user_id:int,key:str):
    rank=get_admin_rank(user_id)
    if rank>=8: return True
    return rank>0 and db.has_permission(rank,key)

def _target_rank(target_id:int):
    return get_admin_rank(target_id)

def _can_target(actor_id:int,target_id:int):
    ar=get_admin_rank(actor_id); tr=get_admin_rank(target_id)
    return ar>=8 or (ar>tr and tr<8)

COMMAND_PERMISSION={"warn":"moderation","unwarn":"moderation","warnings":"moderation","warns":"moderation","mute":"moderation","unmute":"moderation","ban":"moderation","unban":"moderation","kick":"moderation","promote":"ranks","demote":"ranks","role":"ranks","setrole":"ranks","admins":"ranks","shopadd":"shop","shopremove":"shop","shopedit":"shop","shopphoto":"shop","shopcat":"shop","shoprequests":"shop_approve","training":"training","report":"reports","logs":"logs","exportlogs":"logs"}
class V29PermissionMiddleware(BaseMiddleware):
    async def __call__(self,handler,event,data):
        if isinstance(event,Message) and event.text and event.text.startswith('/'):
            cmd=event.text.split()[0][1:].split('@')[0].lower(); key=COMMAND_PERMISSION.get(cmd)
            if key and not permission(event.from_user.id,key): await event.answer('🔒 Недостаточно прав для этой команды.'); return None
        return await handler(event,data)
dp.message.middleware(V29PermissionMiddleware())

def _duration_seconds(raw:str):
    raw=(raw or "").strip().lower()
    if not raw: return None
    m=re.fullmatch(r"(\d+)\s*(s|сек|секунд|m|min|мин|минут|h|ч|час|часы|d|д|дн|дней)?",raw)
    if not m: return None
    n=int(m.group(1)); u=m.group(2) or "m"
    return n*({"s":1,"сек":1,"секунд":1,"m":60,"min":60,"мин":60,"минут":60,"h":3600,"ч":3600,"час":3600,"часы":3600,"d":86400,"д":86400,"дн":86400,"дней":86400}[u])

async def _notify_target(target_id:int,text:str):
    try: await bot.send_message(int(target_id),text)
    except Exception: pass

async def _apply_warn(message, target_id, reason, actor_id):
    if not permission(actor_id,"moderation") or not _can_target(actor_id,target_id): return False,"❌ Недостаточно прав для этого пользователя."
    count=db.add_warning(message.chat.id,target_id,actor_id,reason or "Без причины")
    db.add_moderation_log(message.chat.id,target_id,actor_id,"warn",reason or "Без причины")
    db.log("warn",actor_id,{"target_id":target_id,"reason":reason,"active":count})
    if count>=AUTO_BAN_WARNINGS:
        await bot.ban_chat_member(message.chat.id,target_id)
        db.set_ban(message.chat.id,target_id,actor_id,"Автоматически: достигнут порог 7 активных варнов")
        db.add_moderation_log(message.chat.id,target_id,actor_id,"auto_ban","Достигнут порог 7 активных варнов")
        await _notify_target(target_id,"🔨 Ты получил бан: достигнут порог 7 активных предупреждений.")
    elif count>=AUTO_MUTE_WARNINGS:
        await bot.restrict_chat_member(message.chat.id,target_id,permissions=ChatPermissions(can_send_messages=False))
        db.add_moderation_log(message.chat.id,target_id,actor_id,"auto_mute","Достигнут порог 5 активных варнов")
        await _notify_target(target_id,"🔇 Ты получил бессрочный мут: достигнут порог 5 активных предупреждений.")
    else:
        await _notify_target(target_id,f"⚠️ Тебе выдано предупреждение. Активных: {count}. Причина: {reason or 'не указана'}")
    return True,f"⚠️ Предупреждение выдано. Активных: <b>{count}</b>"

async def _resolve_target_and_reason(message,text,action):
    parts=(text or "").split()
    token=None
    if message.reply_to_message and message.reply_to_message.from_user: token=str(message.reply_to_message.from_user.id)
    else:
        for x in parts[1:]:
            if x.startswith("@") or x.isdigit(): token=x; break
    target=await resolve_moderation_target(message,token)
    if not target: return None,"Не найден пользователь. Используй Reply, @username или Telegram ID."
    target_id=int(target.id if hasattr(target,"id") else target["telegram_id"])
    reason=(text or "").strip()
    reason=re.sub(r"^[/!]?\S+\s*", "", reason, count=1).strip()
    if token: reason=re.sub(r"^@?\w+\s*", "", reason, count=1).strip()
    return (target_id,reason),None

@dp.message(Command("warn"))
async def v29_warn(message:Message):
    if not permission(message.from_user.id,"moderation"): await message.answer("❌ Недостаточно прав."); return
    resolved,err=await _resolve_target_and_reason(message,message.text,"warn")
    if err: await message.answer("❌ "+err); return
    ok,text=await _apply_warn(message,resolved[0],resolved[1],message.from_user.id); await message.answer(text)

@dp.message(Command("unwarn"))
async def v29_unwarn(message:Message):
    if not permission(message.from_user.id,"moderation"): await message.answer("❌ Недостаточно прав."); return
    resolved,err=await _resolve_target_and_reason(message,message.text,"unwarn")
    if err: await message.answer("❌ "+err); return
    tid=resolved[0]
    if not _can_target(message.from_user.id,tid): await message.answer("❌ Недостаточно прав для этого пользователя."); return
    rows=db.get_warnings(message.chat.id,tid,1000)
    if not rows: await message.answer("📭 Активных предупреждений нет."); return
    rid=int(rows[0]["id"]); db.conn.execute("UPDATE moderation_warnings SET active=0 WHERE id=?",(rid,)); db.conn.commit()
    db.add_moderation_log(message.chat.id,tid,message.from_user.id,"unwarn",resolved[1]); db.log("unwarn",message.from_user.id,{"target_id":tid,"warning_id":rid})
    await message.answer("♻️ Предупреждение снято. История сохранена.")

@dp.message(Command("warnings","warns"))
async def v29_warnings(message:Message):
    tid=message.from_user.id
    resolved,err=await _resolve_target_and_reason(message,message.text,"warnings")
    if not err and resolved: tid=resolved[0]
    rows=db.get_warnings(message.chat.id,tid,1000)
    allrows=db.conn.execute("SELECT * FROM moderation_warnings WHERE chat_id=? AND user_id=? ORDER BY id DESC",(message.chat.id,tid)).fetchall()
    active=sum(1 for r in allrows if int(r["active"]))
    lines=["⚠️ <b>ПРЕДУПРЕЖДЕНИЯ</b>",f"Активных: <b>{active}</b>",f"Всего выдано: <b>{len(allrows)}</b>",""]
    for i,r in enumerate(allrows,1): lines += [f"{i}. ⚠️ {html.escape(r['reason'])}",f"   Выдал: <code>{r['admin_id']}</code>",f"   {r['created_at']}",f"   Статус: {'активно' if r['active'] else 'снято'}"]
    await message.answer("\n".join(lines[:120]))

@dp.message(Command("mute"))
async def v29_mute(message:Message):
    if not permission(message.from_user.id,"moderation"): await message.answer("❌ Недостаточно прав."); return
    resolved,err=await _resolve_target_and_reason(message,message.text,"mute")
    if err: await message.answer("❌ "+err); return
    tid,raw=resolved
    if not _can_target(message.from_user.id,tid): await message.answer("❌ Недостаточно прав для этого пользователя."); return
    parts=(message.text or "").split(); dur=None
    for x in parts:
        if _duration_seconds(x): dur=_duration_seconds(x); break
    until=datetime.now(ZoneInfo(TIMEZONE))+timedelta(seconds=dur) if dur else None
    await bot.restrict_chat_member(message.chat.id,tid,permissions=ChatPermissions(can_send_messages=False),until_date=until)
    db.add_moderation_log(message.chat.id,tid,message.from_user.id,"mute",raw, f"{dur}s" if dur else "без срока")
    await _notify_target(tid,f"🔇 Ты получил мут{' на '+str(dur)+' сек.' if dur else ' бессрочно'}.")
    await message.answer(f"🔇 Мут выдан{' до '+until.strftime('%d.%m.%Y %H:%M') if until else ' бессрочно'}.")

@dp.message(Command("unmute"))
async def v29_unmute(message:Message):
    if not permission(message.from_user.id,"moderation"): await message.answer("❌ Недостаточно прав."); return
    resolved,err=await _resolve_target_and_reason(message,message.text,"unmute")
    if err: await message.answer("❌ "+err); return
    tid=resolved[0]
    if not _can_target(message.from_user.id,tid): await message.answer("❌ Недостаточно прав для этого пользователя."); return
    await bot.restrict_chat_member(message.chat.id,tid,permissions=ChatPermissions(can_send_messages=True,can_send_photos=True,can_send_videos=True,can_send_documents=True,can_send_other_messages=True,can_add_web_page_previews=True))
    db.add_moderation_log(message.chat.id,tid,message.from_user.id,"unmute")
    await _notify_target(tid,"🔊 Мут снят. Возможность писать восстановлена."); await message.answer("🔊 Мут снят.")

@dp.message(Command("ban"))
async def v29_ban(message:Message):
    if not permission(message.from_user.id,"moderation"): await message.answer("❌ Недостаточно прав."); return
    resolved,err=await _resolve_target_and_reason(message,message.text,"ban")
    if err: await message.answer("❌ "+err); return
    tid,reason=resolved
    if not _can_target(message.from_user.id,tid): await message.answer("❌ Недостаточно прав для этого пользователя."); return
    await bot.ban_chat_member(message.chat.id,tid)
    db.set_ban(message.chat.id,tid,message.from_user.id,reason or "Без причины")
    db.add_moderation_log(message.chat.id,tid,message.from_user.id,"ban",reason); await _notify_target(tid,f"🔨 Ты забанен. Причина: {reason or 'не указана'}"); await message.answer("🔨 Пользователь забанен.")

@dp.message(Command("unban"))
async def v29_unban(message:Message):
    if not permission(message.from_user.id,"moderation"): await message.answer("❌ Недостаточно прав."); return
    resolved,err=await _resolve_target_and_reason(message,message.text,"unban")
    if err: await message.answer("❌ "+err); return
    tid=resolved[0]
    if not _can_target(message.from_user.id,tid): await message.answer("❌ Недостаточно прав для этого пользователя."); return
    await bot.unban_chat_member(message.chat.id,tid,only_if_banned=True); db.clear_ban(message.chat.id,tid); db.add_moderation_log(message.chat.id,tid,message.from_user.id,"unban"); await message.answer("🔓 Бан снят.")

@dp.message(Command("kick"))
async def v29_kick(message:Message):
    if not permission(message.from_user.id,"moderation"): await message.answer("❌ Недостаточно прав."); return
    resolved,err=await _resolve_target_and_reason(message,message.text,"kick")
    if err: await message.answer("❌ "+err); return
    tid=resolved[0]
    if not _can_target(message.from_user.id,tid): await message.answer("❌ Недостаточно прав для этого пользователя."); return
    await bot.ban_chat_member(message.chat.id,tid); await bot.unban_chat_member(message.chat.id,tid); db.add_moderation_log(message.chat.id,tid,message.from_user.id,"kick"); await message.answer("👢 Пользователь исключён.")

# Text aliases for moderation, including reply-target syntax.
@dp.message(StateFilter(None), F.text.regexp(re.compile(r"^(?:варн|варнить|пред|предупреждение|предупреди|предупредить|выдать\s+(?:пред|варн|предупреждение)|дать\s+(?:пред|варн|предупреждение)|снять\s+(?:варн|пред|предупреждение)|сними\s+(?:варн|пред|предупреждение)|убрать\s+(?:варн|пред|предупреждение)|мут|мутнуть|замутить|замут|дать\s+мут|выдать\s+мут|размут|размутить|размуть|снять\s+мут|бан|забанить|забан|дать\s+бан|выдать\s+бан|разбан|разбанить|разбань|снять\s+бан|кик|кикнуть|выгнать|исключить)(?:\s+.*)?$",re.I)))
async def v29_text_moderation(message:Message):
    text=(message.text or '').strip(); low=text.lower()
    if not permission(message.from_user.id,"moderation"): return
    if low.startswith(('снять варн','снять пред','снять предупреждение','сними варн','сними пред','сними предупреждение','убрать варн','убрать пред','убрать предупреждение')): action='unwarn'
    elif low.startswith(('размут','размуть','снять мут')): action='unmute'
    elif low.startswith(('разбан','разбань','снять бан')): action='unban'
    elif low.startswith(('бан','забан','дать бан','выдать бан')): action='ban'
    elif low.startswith(('кик','выгнать','исключить')): action='kick'
    elif low.startswith(('мут','замут','мутнуть','дать мут','выдать мут')): action='mute'
    else: action='warn'
    resolved,err=await _resolve_target_and_reason(message,text,action)
    if err: await message.answer('❌ '+err); return
    tid,reason=resolved
    if not _can_target(message.from_user.id,tid): await message.answer('❌ Недостаточно прав для этого пользователя.'); return
    if action=='warn': ok,out=await _apply_warn(message,tid,reason,message.from_user.id); await message.answer(out)
    elif action=='unwarn':
        rows=db.get_warnings(message.chat.id,tid,1000)
        if rows: db.conn.execute('UPDATE moderation_warnings SET active=0 WHERE id=?',(int(rows[0]['id']),)); db.conn.commit(); db.add_moderation_log(message.chat.id,tid,message.from_user.id,'unwarn',reason); await message.answer('♻️ Варн снят.')
        else: await message.answer('📭 Активных варнов нет.')
    elif action=='mute': await bot.restrict_chat_member(message.chat.id,tid,permissions=ChatPermissions(can_send_messages=False)); db.add_moderation_log(message.chat.id,tid,message.from_user.id,'mute',reason); await _notify_target(tid,'🔇 Тебе выдан мут.'); await message.answer('🔇 Мут выдан.')
    elif action=='unmute': await bot.restrict_chat_member(message.chat.id,tid,permissions=ChatPermissions(can_send_messages=True,can_send_photos=True,can_send_videos=True,can_send_documents=True,can_send_other_messages=True,can_add_web_page_previews=True)); db.add_moderation_log(message.chat.id,tid,message.from_user.id,'unmute'); await message.answer('🔊 Мут снят.')
    elif action=='ban': await bot.ban_chat_member(message.chat.id,tid); db.set_ban(message.chat.id,tid,message.from_user.id,reason or 'Без причины'); db.add_moderation_log(message.chat.id,tid,message.from_user.id,'ban',reason); await _notify_target(tid,'🔨 Ты забанен.'); await message.answer('🔨 Бан выдан.')
    elif action=='unban': await bot.unban_chat_member(message.chat.id,tid,only_if_banned=True); db.clear_ban(message.chat.id,tid); db.add_moderation_log(message.chat.id,tid,message.from_user.id,'unban'); await message.answer('🔓 Бан снят.')
    else: await bot.ban_chat_member(message.chat.id,tid); await bot.unban_chat_member(message.chat.id,tid); db.add_moderation_log(message.chat.id,tid,message.from_user.id,'kick'); await message.answer('👢 Пользователь исключён.')

@dp.callback_query(F.data == "admin_access_settings")
async def v29_access_settings(callback:CallbackQuery):
    if not owner_only(callback.from_user.id): await callback.answer('🔒 Только Лидер.',show_alert=True); return
    rows=[]
    for role in range(8,0,-1): rows.append([InlineKeyboardButton(text=f"{rank_name(role)}",callback_data=f"access_role|{role}")])
    rows.append([InlineKeyboardButton(text='⬅️ Назад',callback_data='menu_admin')])
    await safe_edit(callback.message,'⚙️ <b>НАСТРОЙКИ ДОСТУПА</b>\n\nЛидер имеет полный доступ и не редактируется. Выбери роль:',reply_markup=InlineKeyboardMarkup(inline_keyboard=rows)); await callback.answer()

@dp.callback_query(F.data.startswith("access_role|"))
async def v29_access_role(callback:CallbackQuery):
    if not owner_only(callback.from_user.id): await callback.answer('Только Лидер.',show_alert=True); return
    role=int(callback.data.split("|",1)[1]); rows=[]
    for key,label in ACCESS_KEYS.items():
        enabled=db.has_permission(role,key) if role<8 else True; mark='✅' if enabled else '❌'; rows.append([InlineKeyboardButton(text=f'{mark} {label}',callback_data=f'access_toggle|{role}|{key}')])
    rows.append([InlineKeyboardButton(text='⬅️ Роли',callback_data='admin_access_settings')])
    await safe_edit(callback.message,f'⚙️ <b>{rank_name(role)}</b>\n\nНажми на разрешение для переключения:',reply_markup=InlineKeyboardMarkup(inline_keyboard=rows)); await callback.answer()

@dp.callback_query(F.data.startswith("access_toggle|"))
async def v29_access_toggle(callback:CallbackQuery):
    if not owner_only(callback.from_user.id): await callback.answer('Только Лидер.',show_alert=True); return
    parts=callback.data.split("|",2)
    if len(parts)!=3:
        await callback.answer("Некорректная команда.",show_alert=True); return
    try: role=int(parts[1])
    except ValueError:
        await callback.answer("Некорректная роль.",show_alert=True); return
    key=parts[2]
    if role < 1 or role > 7 or key not in ACCESS_KEYS:
        await callback.answer("Некорректное разрешение.",show_alert=True); return
    enabled=not db.has_permission(role,key)
    db.set_access_permission(role,key,enabled)
    db.log("access_permission_changed",callback.from_user.id,{"role":role,"key":key,"enabled":enabled})
    await callback.answer("Включено" if enabled else "Выключено")
    await v29_access_role(callback)

@dp.callback_query(F.data == "admin_training")
async def v29_training_panel(callback:CallbackQuery):
    if not permission(callback.from_user.id,'training'): await callback.answer('Нет доступа.',show_alert=True); return
    st=db.get_training_settings(); events=db.conn.execute("SELECT * FROM training_events ORDER BY event_date DESC LIMIT 8").fetchall(); text='🏋️ <b>ТРЕНИРОВКИ</b>\n\n'; text+=f"Статус: {'включены' if st['enabled'] else 'выключены'}\nКаждый понедельник: <b>{st['start_time']} МСК</b>\nФормат: <b>{st['team_size']}×{st['team_size']}</b>\n\n"
    for e in events: text+=f"#{e['id']} — {e['event_date']} {e['event_time']} — {e['status']}\n"
    kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='🟢 Включить',callback_data='training_enable'),InlineKeyboardButton(text='🔴 Выключить',callback_data='training_disable')],[InlineKeyboardButton(text='📝 Создать ближайшую',callback_data='training_create_next')],[InlineKeyboardButton(text='⬅️ Назад',callback_data='menu_admin')]])
    await safe_edit(callback.message,text,reply_markup=kb); await callback.answer()

@dp.callback_query(F.data.in_({'training_enable','training_disable'}))
async def v29_training_toggle(callback:CallbackQuery):
    if not owner_only(callback.from_user.id): await callback.answer('Только Лидер.',show_alert=True); return
    db.set_training_settings(enabled=callback.data=='training_enable'); db.log('training_settings',callback.from_user.id,{'enabled':callback.data=='training_enable'}); await v29_training_panel(callback)

@dp.callback_query(F.data == 'training_create_next')
async def v29_training_next(callback:CallbackQuery):
    if not permission(callback.from_user.id,'training'): await callback.answer('Нет доступа.',show_alert=True); return
    now=datetime.now(ZoneInfo(TIMEZONE)); days=(0-now.weekday())%7; d=now.date()+timedelta(days=days); 
    if d<=now.date(): d+=timedelta(days=7)
    e=db.create_training_event(d.isoformat(),'20:00'); await safe_edit(callback.message,f'🏋️ Тренировка создана: <b>{e["event_date"]} 20:00 МСК</b>\n\nУчастники могут нажать /training или ответить «тренировка да».',reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='⬅️ Назад',callback_data='admin_training')]])); await callback.answer()

@dp.message(Command('trainingset'))
async def v29_trainingset(m):
    if not owner_only(m.from_user.id): await m.answer('🔒 Только Лидер.'); return
    parts=(m.text or '').split()
    if len(parts)<3 or not re.fullmatch(r'\d{2}:\d{2}',parts[1]) or not parts[2].isdigit(): await m.answer('Формат: /trainingset HH:MM РАЗМЕР_КОМАНДЫ'); return
    hh,mm=map(int,parts[1].split(':'))
    if hh>23 or mm>59 or int(parts[2])<1: await m.answer('❌ Некорректные настройки.'); return
    db.set_training_settings(start_time=parts[1],team_size=int(parts[2])); db.log('training_settings',m.from_user.id,{'time':parts[1],'team_size':int(parts[2])}); await m.answer(f'✅ Тренировки: понедельник {parts[1]} МСК, формат {parts[2]}×{parts[2]}.')

@dp.message(Command('training'))
async def v29_training_user(message:Message):
    p=db.get_player_by_telegram(message.from_user.id)
    if not p or int(message.from_user.id)==int(OWNER_ID): await message.answer('❌ Участвовать могут только зарегистрированные ученики. Лидер не участвует.'); return
    st=db.get_training_settings(); now=datetime.now(ZoneInfo(TIMEZONE)); days=(0-now.weekday())%7; d=now.date()+timedelta(days=days)
    if d<now.date() or (d==now.date() and now.strftime('%H:%M')>='20:00'): d+=timedelta(days=7)
    e=db.create_training_event(d.isoformat(),st['start_time']); rows=db.get_training_participants(e['id']);
    await message.answer(f'🏋️ <b>Тренировка</b>\n{e["event_date"]} {e["event_time"]} МСК\nУчастников: {sum(1 for r in rows if r["status"]=="accepted")}\n\nВыбери:',reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='✅ Участвую',callback_data=f'training_join_{e["id"]}'),InlineKeyboardButton(text='❌ Отказываюсь',callback_data=f'training_decline_{e["id"]}')]]))

@dp.callback_query(F.data.startswith('training_join_'))
async def v29_training_join(callback:CallbackQuery):
    p=db.get_player_by_telegram(callback.from_user.id)
    if not p or callback.from_user.id==OWNER_ID: await callback.answer('Участие недоступно.',show_alert=True); return
    eid=int(callback.data.rsplit('_',1)[1]); db.add_training_participant(eid,callback.from_user.id,p['player_id'],'accepted'); await callback.answer('Ты записан!');
    try: await callback.message.edit_text('✅ Ты записан на тренировку.')
    except Exception: pass

class TrainingDeclineState(StatesGroup): waiting_reason=State()

@dp.callback_query(F.data.startswith('training_decline_'))
async def v29_training_decline(callback:CallbackQuery,state:FSMContext):
    await state.set_state(TrainingDeclineState.waiting_reason); await state.update_data(training_event_id=int(callback.data.rsplit('_',1)[1])); await callback.message.answer('❌ Напиши причину отказа:'); await callback.answer()

@dp.message(TrainingDeclineState.waiting_reason)
async def v29_training_decline_reason(message:Message,state:FSMContext):
    p=db.get_player_by_telegram(message.from_user.id); data=await state.get_data();
    if p: db.add_training_participant(data['training_event_id'],message.from_user.id,p['player_id'],'declined',message.text or '')
    await state.clear(); await message.answer('❌ Отказ записан с причиной.')
...
class ShopCategoryState(StatesGroup):
    waiting_name = State()

def _shop_admin_back():
    return InlineKeyboardButton(text="⬅️ Назад", callback_data="admin_shop_settings")

@dp.callback_query(F.data == 'admin_shop_settings')
async def v29_shop_admin(callback:CallbackQuery):
    if not permission(callback.from_user.id,'shop'):
        await callback.answer('Нет доступа.',show_alert=True); return
    cats=db.get_shop_categories(False); products=db.get_shop_products(False)
    text=(
        '🛒 <b>НАСТРОЙКА МАГАЗИНА</b>\n\n'
        f'📂 Категорий: <b>{len(cats)}</b>\n'
        f'📦 Товаров: <b>{len(products)}</b>\n\n'
        'Здесь можно полностью управлять каталогом.'
    )
    kb=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='➕ Добавить товар',callback_data='shop_admin_add')],
        [InlineKeyboardButton(text='📂 Категории',callback_data='shop_admin_categories')],
        [InlineKeyboardButton(text='📦 Товары',callback_data='shop_admin_products')],
        [InlineKeyboardButton(text='📋 Заявки на покупки',callback_data='shop_admin_requests')],
        [_shop_admin_back().copy(update={"callback_data":"menu_admin"})]
    ])
    await safe_edit(callback.message,text,reply_markup=kb); await callback.answer()

@dp.callback_query(F.data == 'shop_admin_add')
async def v29_shop_admin_add(callback:CallbackQuery,state:FSMContext):
    if not permission(callback.from_user.id,'shop'):
        await callback.answer('Нет доступа.',show_alert=True); return
    cats=db.get_shop_categories(False)
    cat_text="\n".join(f"#{c['category_id']} — {html.escape(c['name'])}" for c in cats) or "Категорий пока нет."
    await state.set_state(ShopCreateState.waiting_data)
    await state.update_data(chat_id=callback.message.chat.id)
    await callback.message.answer(
        "➕ <b>ДОБАВЛЕНИЕ ТОВАРА</b>\n\n"
        "Отправь одной строкой:\n"
        "<code>ЦЕНА | НАЗВАНИЕ | ОПИСАНИЕ | ID_КАТЕГОРИИ</code>\n\n"
        f"<b>Категории:</b>\n{cat_text}\n\n"
        "ID категории можно оставить пустым.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='❌ Отмена',callback_data='shop_admin_cancel')]])
    )
    await callback.answer()

@dp.callback_query(F.data == 'shop_admin_cancel')
async def v29_shop_admin_cancel(callback:CallbackQuery,state:FSMContext):
    await state.clear()
    await safe_edit(callback.message,"🛒 <b>Настройка магазина</b>",reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='➕ Добавить товар',callback_data='shop_admin_add')],
        [InlineKeyboardButton(text='📂 Категории',callback_data='shop_admin_categories')],
        [InlineKeyboardButton(text='📦 Товары',callback_data='shop_admin_products')],
...
@dp.message(ShopCreateState.waiting_data)
async def v29_shop_admin_add_data(message:Message,state:FSMContext):
    if not permission(message.from_user.id,'shop'):
        await state.clear(); return
    raw=(message.text or "").strip()
    vals=[v.strip() for v in raw.split("|")]
    if len(vals)<2 or not vals[0].isdigit() or int(vals[0])<=0 or not vals[1]:
        await message.answer("❌ Формат: <code>ЦЕНА | НАЗВАНИЕ | ОПИСАНИЕ | ID_КАТЕГОРИИ</code>")
        return
    price=int(vals[0]); name=vals[1]; desc=vals[2] if len(vals)>2 else ""
    cat=int(vals[3]) if len(vals)>3 and vals[3].isdigit() else None
    if cat is not None and not db.conn.execute("SELECT 1 FROM shop_categories WHERE category_id=?",(cat,)).fetchone():
        await message.answer("❌ Такая категория не существует."); return
    pid=db.add_shop_product(name,price,desc)
    if cat is not None: db.update_shop_product(pid,category_id=cat)
    await state.clear()
    await message.answer(f"✅ <b>Товар #{pid} добавлен.</b>\n📦 {html.escape(name)}\n💰 {price} 🪙")

@dp.callback_query(F.data == 'shop_admin_categories')
async def v29_shop_categories(callback:CallbackQuery):
    if not permission(callback.from_user.id,'shop'):
        await callback.answer('Нет доступа.',show_alert=True); return
    rows=[[InlineKeyboardButton(text=f'📂 {c["name"]}',callback_data=f'shop_cat_db_{c["category_id"]}')] for c in db.get_shop_categories(False)]
    rows.append([InlineKeyboardButton(text='➕ Добавить категорию',callback_data='shop_admin_add_category')])
    rows.append([_shop_admin_back()])
    await safe_edit(callback.message,'📂 <b>КАТЕГОРИИ</b>\n\nВыбери категорию или создай новую.',reply_markup=InlineKeyboardMarkup(inline_keyboard=rows)); await callback.answer()

@dp.callback_query(F.data == 'shop_admin_add_category')
async def v29_shop_category_add(callback:CallbackQuery,state:FSMContext):
    if not permission(callback.from_user.id,'shop'):
        await callback.answer('Нет доступа.',show_alert=True); return
    await state.set_state(ShopCategoryState.waiting_name)
    await callback.message.answer("📂 Отправь название новой категории:",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='❌ Отмена',callback_data='shop_admin_cancel')]]))
    await callback.answer()

@dp.message(ShopCategoryState.waiting_name)
async def v29_shop_category_add_data(message:Message,state:FSMContext):
    if not permission(message.from_user.id,'shop'):
        await state.clear(); return
    name=(message.text or "").strip()
    if not name:
        await message.answer("❌ Название не может быть пустым."); return
    try: cid=db.add_shop_category(name)
    except Exception as e:
        await message.answer(f"❌ Не удалось создать категорию: {html.escape(str(e))}"); return
    await state.clear(); await message.answer(f"✅ Категория #{cid} «{html.escape(name)}» создана.")

@dp.callback_query(F.data == 'shop_admin_products')
async def v29_shop_products(callback:CallbackQuery):
    if not permission(callback.from_user.id,'shop'):
        await callback.answer('Нет доступа.',show_alert=True); return
    rows=[]
    products=db.get_shop_products(False)
    for r in products:
        status="✅" if int(r["active"]) else "🙈"
        rows.append([InlineKeyboardButton(text=f'{status} 📦 {r["name"]} — {r["price"]} 🪙',callback_data=f'shop_product_{r["product_id"]}')])
    rows.append([InlineKeyboardButton(text='➕ Добавить товар',callback_data='shop_admin_add')])
    rows.append([_shop_admin_back()])
    await safe_edit(callback.message,'📦 <b>ТОВАРЫ</b>\n\n🙈 — скрыт из пользовательского магазина.',reply_markup=InlineKeyboardMarkup(inline_keyboard=rows)); await callback.answer()

@dp.callback_query(F.data.startswith('shop_edit_'))
async def v29_shop_edit(callback:CallbackQuery,state:FSMContext):
    if not permission(callback.from_user.id,'shop'):
        await callback.answer('Нет доступа.',show_alert=True); return
    try: pid=int(callback.data.rsplit('_',1)[1])
    except ValueError: await callback.answer("Неверный товар.",show_alert=True); return
    row=next((r for r in db.get_shop_products(False) if int(r["product_id"])==pid),None)
    if not row: await callback.answer("Товар не найден.",show_alert=True); return
    await state.set_state(ShopEditState.waiting_data); await state.update_data(product_id=pid)
    await callback.message.answer(
        f"✏️ <b>Редактирование товара #{pid}</b>\n\n"
        f"Текущее: <b>{html.escape(row['name'])}</b> — {row['price']} 🪙\n\n"
        "Отправь:\n<code>ЦЕНА | НАЗВАНИЕ | ОПИСАНИЕ | ID_КАТЕГОРИИ</code>\n"
        "Пустые поля можно оставить через |."
    )
    await callback.answer()

@dp.message(ShopEditState.waiting_data)
async def v29_shop_edit_data(message:Message,state:FSMContext):
    if not permission(message.from_user.id,'shop'):
        await state.clear(); return
    data=await state.get_data(); pid=int(data["product_id"])
    row=next((r for r in db.get_shop_products(False) if int(r["product_id"])==pid),None)
    if not row: await state.clear(); await message.answer("❌ Товар не найден."); return
    vals=[v.strip() for v in (message.text or "").split("|")]
    if len(vals)<2:
        await message.answer("❌ Формат: <code>ЦЕНА | НАЗВАНИЕ | ОПИСАНИЕ | ID_КАТЕГОРИИ</code>"); return
    price=int(vals[0]) if vals[0].isdigit() and int(vals[0])>0 else int(row["price"])
    name=vals[1] or row["name"]; desc=vals[2] if len(vals)>2 else row["description"]
    cat=row["category_id"]
    if len(vals)>3:
        if vals[3].isdigit(): cat=int(vals[3])
        elif vals[3]=="": cat=None
    if cat is not None and not db.conn.execute("SELECT 1 FROM shop_categories WHERE category_id=?",(cat,)).fetchone():
        await message.answer("❌ Такая категория не существует."); return
    db.update_shop_product(pid,name=name,price=price,description=desc,category_id=cat)
    await state.clear(); await message.answer(f"✅ Товар #{pid} обновлён.")

@dp.callback_query(F.data.startswith('shop_hide_'))
async def v29_shop_hide(callback:CallbackQuery):
    if not permission(callback.from_user.id,'shop'):
        await callback.answer('Нет доступа.',show_alert=True); return
    try: pid=int(callback.data.rsplit('_',1)[1])
    except ValueError: await callback.answer("Неверный товар.",show_alert=True); return
    ...
    db.update_shop_product(pid,active=new_active)
    await callback.answer("Товар показан." if new_active else "Товар скрыт.")
    await v29_shop_products(callback)

@dp.callback_query(F.data == 'shop_admin_requests')
async def v29_shop_admin_requests(callback:CallbackQuery):
    if not permission(callback.from_user.id,'shop_approve'):
        await callback.answer('Нет права одобрять покупки.',show_alert=True); return
    rows=db.get_shop_requests('pending',50)
    if not rows:
        text="📋 <b>ЗАЯВКИ НА ПОКУПКИ</b>\n\nНет ожидающих заявок."
    else:
        text="📋 <b>ЗАЯВКИ НА ПОКУПКИ</b>\n\nВыбери заявку:"
    kb_rows=[]
    for r in rows:
        kb_rows.append([InlineKeyboardButton(text=f"#{r['request_id']} — {r['request_type']} — {r['coins_cost']} 🪙",callback_data=f"shop_approve_{r['request_id']}")])
    kb_rows.append([_shop_admin_back()])
    await safe_edit(callback.message,text,reply_markup=InlineKeyboardMarkup(inline_keyboard=kb_rows)); await callback.answer()

@dp.message(Command('report'))
async def v29_report(message:Message):
    resolved,err=await _resolve_target_and_reason(message,message.text,'report')
    if err or not resolved or not resolved[1]: await message.answer('Формат: /report @user причина или ответь на сообщение.'); return
    tid,reason=resolved; rid=db.create_report(message.chat.id,message.from_user.id,tid,reason); db.log('report_created',message.from_user.id,{'report_id':rid,'target_id':tid,'reason':reason})
    for uid in [int(OWNER_ID)]:
        try: await bot.send_message(uid,f'📩 <b>ЖАЛОБА #{rid}</b>\n👤 От: <code>{message.from_user.id}</code>\n🎯 На: <code>{tid}</code>\nПричина: {html.escape(reason)}')
        except Exception: pass
    await message.answer(f'✅ Жалоба #{rid} создана и передана администрации.')

@dp.callback_query(F.data == 'admin_reports')
async def v29_reports(callback:CallbackQuery):
    if not permission(callback.from_user.id,'reports'): await callback.answer('Нет доступа.',show_alert=True); return
    rows=db.get_reports(); text='📩 <b>ЖАЛОБЫ</b>\n\n' + ('\n'.join(f'#{r["id"]} → {r["target_id"]}: {html.escape(r["reason"])}' for r in rows) or 'Нет новых жалоб.'); await safe_edit(callback.message,text,reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='⬅️ Назад',callback_data='menu_admin')]])); await callback.answer()

async def v29_log_cleanup_loop():
    while True:
        try:
            if LOG_CLEANUP_ENABLED:
                db.conn.execute("DELETE FROM logs WHERE datetime(created_at) < datetime('now', ?)",(f'-{int(LOG_RETENTION_DAYS)} days',)); db.conn.execute("DELETE FROM moderation_logs WHERE datetime(created_at) < datetime('now', ?)",(f'-{int(LOG_RETENTION_DAYS)} days',)); db.conn.commit()
        except Exception: logger.exception('Ошибка очистки логов')
        await asyncio.sleep(86400)

async def v29_daily_profile_sync_loop():
    """Refresh every registered FF profile once per day and remove members who left our guild."""
    last_run = None
    while True:
        try:
            now = datetime.now(ZoneInfo(TIMEZONE))
            today = now.date().isoformat()
            # Run once shortly after 04:30 Moscow each day; also catch up after downtime.
            if now.hour >= 4 and last_run != today:
                players = list(db.get_all_players())
                logger.info("Starting daily Free Fire profile sync for %s players", len(players))
                for p in players:
                    uid = str(p["player_id"])
                    tg_id = p["telegram_id"]
                    try:
                        profile = await ff_client.get_player_profile(uid, FF_REGION)
                        if not profile:
                            logger.warning("Profile sync skipped for UID %s: API returned no profile", uid)
                            continue
                        actual_guild = str(profile.guild_id) if profile.guild_id else None
                        expected_guild = str(FF_GUILD_ID) if FF_GUILD_ID else None
                        # Never treat an API outage/empty guild as proof of leaving. Only a real
                        # profile response with a different guild ID triggers removal.
                        if expected_guild and actual_guild != expected_guild:
                            if tg_id:
                                try:
                                    await bot.send_message(
                                        int(tg_id),
                                        "🚪 Ты вышел из гильдии в Free Fire.\n\n"
                                        "❌ Бот удалит тебя из чата гильдии и базы данных.\n"
                                        "Причина: выход из гильдии в игре."
                                    )
                                except Exception:
                                    pass
                                try:
                                    await bot.send_message(
                                        GUILD_CHAT_ID,
                                        f"🚪 {mention_user(int(tg_id), p['nick'] or uid)} исключён из чата.\n"
                                        "Причина: выход из гильдии в игре."
                                    )
                                except Exception:
                                    pass
                                try:
                                    await bot.ban_chat_member(GUILD_CHAT_ID, int(tg_id))
                                    await bot.unban_chat_member(GUILD_CHAT_ID, int(tg_id), only_if_banned=True)
                                except Exception:
                                    logger.exception("Не удалось исключить Telegram %s после выхода из FF-гильдии", tg_id)
                            db.delete_player(uid)
                            logger.info("Removed UID %s after leaving guild %s", uid, expected_guild)
                            continue
                        # Profile still belongs to our guild: refresh API-owned fields only.
                        db.update_player_profile(uid, ff_client.profile_to_dict(profile))
                    except Exception:
                        logger.exception("Daily profile sync failed for UID %s", uid)
                last_run = today
        except asyncio.CancelledError:
            raise
        except Exception:
            logger.exception("Ошибка ежедневного обновления профилей")
        await asyncio.sleep(30)


async def v29_training_scheduler():
    while True:
        try:
            st=db.get_training_settings(); now=datetime.now(ZoneInfo(TIMEZONE))
            if int(st['enabled']) and now.weekday()==0 and now.strftime('%H:%M')==st['start_time']:
                e=db.create_training_event(now.date().isoformat(),st['start_time']); players=[p for p in db.get_all_players() if p['telegram_id'] and int(p['telegram_id'])!=int(OWNER_ID)]
                for p in players:
                    await _notify_target(int(p['telegram_id']),f'🏋️ Тренировка сегодня в {st["start_time"]} МСК. Напиши /training для участия.')
                await asyncio.sleep(61)
        except Exception: logger.exception('Ошибка training scheduler')
        await asyncio.sleep(20)


@dp.callback_query(F.data.startswith('profile_history_'))
async def v29_profile_history(c):
    tid=int(c.data.rsplit('_',1)[1]); rows=db.conn.execute('SELECT * FROM moderation_warnings WHERE chat_id=? AND user_id=? ORDER BY id DESC',(GUILD_CHAT_ID,tid)).fetchall(); ban=db.get_ban(GUILD_CHAT_ID,tid); lines=['⚠️ <b>ИСТОРИЯ НАРУШЕНИЙ</b>','']
    for r in rows: lines.append(f"⚠️ {html.escape(r['reason'])} — {'активно' if r['active'] else 'снято'} — {r['created_at']}")
    if ban: lines.append(f"🔨 Бан: {'активен' if ban['active'] else 'снят'}")
    await safe_edit(c.message,'\n'.join(lines) if len(lines)>2 else '📭 История нарушений пуста.',reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='⬅️ Назад',callback_data='menu_profile')]])); await c.answer()

@dp.message(Command('exportlogs'))
async def v29_export_logs(m):
    if not permission(m.from_user.id,'logs'): await m.answer('🔒 Нет доступа.'); return
    import csv
    from io import StringIO
    rows=db.conn.execute('SELECT * FROM moderation_logs ORDER BY id DESC').fetchall(); out=StringIO(); w=csv.writer(out); w.writerow(['id','chat_id','target_id','actor_id','action','reason','duration','created_at']); w.writerows([list(r) for r in rows]); data=out.getvalue().encode('utf-8-sig'); await m.answer_document(BufferedInputFile(data,filename='vaka_moderation_logs.csv'))
    try:
        from openpyxl import Workbook
        import tempfile
        wb=Workbook(); ws=wb.active; ws.title='Moderation'
        ws.append(['id','chat_id','target_id','actor_id','action','reason','duration','created_at'])
        for r in rows: ws.append(list(r))
        tmp=tempfile.NamedTemporaryFile(suffix='.xlsx',delete=False); tmp.close(); wb.save(tmp.name)
        await m.answer_document(FSInputFile(tmp.name),caption='📊 Excel-экспорт журнала')
        Path(tmp.name).unlink(missing_ok=True)
    except Exception as e: logger.warning('Excel export unavailable: %s',e)

@dp.message(StateFilter(None), F.text.regexp(re.compile(r'^(?:тренировка|тренировки|тренировка\s+(?:да|нет)|тренировка\s+(?:участвую|отказываюсь))(?:\s+.*)?$',re.I)))
async def v29_training_text_alias(message:Message):
    low=(message.text or '').strip().lower()
    if low in ('тренировка да','тренировка участвую','тренировка'):
        await v29_training_user(message)
    elif low.startswith(('тренировка нет','тренировка отказываюсь')):
        await message.answer('❌ Для отказа нажми кнопку «Отказаться» после /training, чтобы бот запросил причину.')
...
async def send_rules_bundle(message: Message, kind: str, reply_markup=None):
    """Send rules as an image only, so the chat is not flooded with duplicate text."""
    if kind == "kv":
    ...
    try:
        if image.exists():
            # Remove the old menu message when possible: rules are represented by one photo only.
            try:
                await bot.delete_message(message.chat.id, message.message_id)
            except Exception:
                pass
            await bot.send_photo(message.chat.id, FSInputFile(str(image)), caption=title, reply_markup=reply_markup)
            return
        # Safe fallback only if the asset is missing.
...
async def callback_kv_proposal_v71(callback: CallbackQuery):
    ...
    if callback.data.startswith("v71_kv_decline_"):
        ...
        if row["proposer_id"]:
            try:
                await bot.send_message(int(row["proposer_id"]), f"❌ Твоё предложение КВ #{kid} отклонено руководством.")
            except Exception:
                pass
        await callback.answer("КВ отклонено")
        return
...
async def main():
    ...
    asyncio.create_task(v29_training_scheduler())
    asyncio.create_task(v29_daily_profile_sync_loop())
    asyncio.create_task(v29_log_cleanup_loop())
