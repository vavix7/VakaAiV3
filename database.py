...
class Database:
    ...
    def _create_tables(self):
        ...
        # V2.9 additive management/training/access/reporting storage.
        c.execute("""CREATE TABLE IF NOT EXISTS access_permissions (
            role_level INTEGER NOT NULL, permission_key TEXT NOT NULL, enabled INTEGER NOT NULL DEFAULT 0,
            PRIMARY KEY(role_level, permission_key)
        )""")
        c.execute("""CREATE TABLE IF NOT EXISTS shop_categories (
            category_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE, description TEXT DEFAULT '', active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL, updated_at TEXT NOT NULL
        )""")
        self._add_missing_columns("shop_products", {"category_id":"INTEGER", "photo_file_id":"TEXT", "action_key":"TEXT"})
        c.execute("""CREATE TABLE IF NOT EXISTS training_settings (
            id INTEGER PRIMARY KEY CHECK(id=1), enabled INTEGER NOT NULL DEFAULT 1, weekday INTEGER NOT NULL DEFAULT 0, start_time TEXT NOT NULL DEFAULT '20:00', team_size INTEGER NOT NULL DEFAULT 4, updated_at TEXT NOT NULL
        )""")
        c.execute("""CREATE TABLE IF NOT EXISTS training_events (
            id INTEGER PRIMARY KEY AUTOINCREMENT, event_date TEXT NOT NULL, event_time TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'open', created_at TEXT NOT NULL, updated_at TEXT NOT NULL, UNIQUE(event_date,event_time)
        )""")
        c.execute("""CREATE TABLE IF NOT EXISTS training_participants (
            event_id INTEGER NOT NULL, telegram_id INTEGER NOT NULL, player_id TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'accepted', reason TEXT, created_at TEXT NOT NULL,
            PRIMARY KEY(event_id,telegram_id), FOREIGN KEY(event_id) REFERENCES training_events(id) ON DELETE CASCADE
        )""")
        c.execute("""CREATE TABLE IF NOT EXISTS moderation_logs (
            id INTEGER PRIMARY KEY AUTOINCREMENT, chat_id INTEGER NOT NULL, target_id INTEGER, actor_id INTEGER, action TEXT NOT NULL, reason TEXT, duration TEXT, created_at TEXT NOT NULL
        )""")
        c.execute("""CREATE TABLE IF NOT EXISTS reports (
            id INTEGER PRIMARY KEY AUTOINCREMENT, chat_id INTEGER NOT NULL, reporter_id INTEGER NOT NULL, target_id INTEGER NOT NULL, reason TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'pending', created_at TEXT NOT NULL, handled_at TEXT, handled_by INTEGER
        )""")
        c.execute("CREATE INDEX IF NOT EXISTS idx_reports_status ON reports(status,id DESC)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_modlogs_created ON moderation_logs(created_at DESC)")
        c.execute("""
    ...
    # ---------------- V2.9 access/shop/training/moderation ----------------
    def seed_access_permissions(self, defaults):
        for role, keys in defaults.items():
            for key, enabled in keys.items():
                self.conn.execute("INSERT OR IGNORE INTO access_permissions(role_level,permission_key,enabled) VALUES(?,?,?)", (int(role), str(key), int(bool(enabled))))
        self.conn.commit()

    def get_access_permissions(self, role_level):
        return self.conn.execute("SELECT permission_key,enabled FROM access_permissions WHERE role_level=? ORDER BY permission_key", (int(role_level),)).fetchall()

    def set_access_permission(self, role_level, key, enabled):
        if int(role_level)==8: raise ValueError("Права Лидера нельзя изменять")
        self.conn.execute("INSERT INTO access_permissions(role_level,permission_key,enabled) VALUES(?,?,?) ON CONFLICT(role_level,permission_key) DO UPDATE SET enabled=excluded.enabled", (int(role_level),str(key),int(bool(enabled))))
        self.conn.commit()

    def has_permission(self, role_level, key):
        if int(role_level)>=8: return True
        row=self.conn.execute("SELECT enabled FROM access_permissions WHERE role_level=? AND permission_key=?",(int(role_level),str(key))).fetchone()
        return bool(row and int(row[0]))

    def add_shop_category(self,name,description=''):
        now=self._now(); cur=self.conn.execute("INSERT INTO shop_categories(name,description,created_at,updated_at) VALUES(?,?,?,?)",(str(name).strip(),str(description).strip(),now,now)); self.conn.commit(); return cur.lastrowid
    def get_shop_categories(self,active_only=True):
        q="SELECT * FROM shop_categories" + (" WHERE active=1" if active_only else "") + " ORDER BY category_id"; return self.conn.execute(q).fetchall()
    def update_shop_product(self,pid,name=None,price=None,description=None,category_id=None,photo_file_id=None,active=None,action_key=None):
        row=self.conn.execute("SELECT * FROM shop_products WHERE product_id=?",(int(pid),)).fetchone()
        if not row: return False
        vals=[name if name is not None else row['name'], price if price is not None else row['price'], description if description is not None else row['description'], category_id if category_id is not None else row['category_id'], photo_file_id if photo_file_id is not None else row['photo_file_id'], active if active is not None else row['active'], action_key if action_key is not None else row['action_key'], self._now(), int(pid)]
        self.conn.execute("UPDATE shop_products SET name=?,price=?,description=?,category_id=?,photo_file_id=?,active=?,action_key=?,updated_at=? WHERE product_id=?",vals); self.conn.commit(); return True
    def create_training_event(self,event_date,event_time):
        now=self._now(); cur=self.conn.execute("INSERT OR IGNORE INTO training_events(event_date,event_time,created_at,updated_at) VALUES(?,?,?,?)",(event_date,event_time,now,now)); self.conn.commit(); row=self.conn.execute("SELECT * FROM training_events WHERE event_date=? AND event_time=?",(event_date,event_time)).fetchone(); return row
    def get_training_settings(self):
        row=self.conn.execute("SELECT * FROM training_settings WHERE id=1").fetchone()
        if not row:
            now=self._now(); self.conn.execute("INSERT INTO training_settings(id,updated_at) VALUES(1,?)",(now,)); self.conn.commit(); row=self.conn.execute("SELECT * FROM training_settings WHERE id=1").fetchone()
        return row
    def set_training_settings(self,enabled=None,weekday=None,start_time=None,team_size=None):
        r=self.get_training_settings(); vals=(int(r['enabled'] if enabled is None else enabled),int(r['weekday'] if weekday is None else weekday),str(r['start_time'] if start_time is None else start_time),int(r['team_size'] if team_size is None else team_size),self._now()); self.conn.execute("UPDATE training_settings SET enabled=?,weekday=?,start_time=?,team_size=?,updated_at=? WHERE id=1",vals); self.conn.commit()
    def get_training_event(self,eid): return self.conn.execute("SELECT * FROM training_events WHERE id=?",(int(eid),)).fetchone()
    def add_training_participant(self,eid,telegram_id,player_id,status='accepted',reason=None):
        self.conn.execute("INSERT OR REPLACE INTO training_participants(event_id,telegram_id,player_id,status,reason,created_at) VALUES(?,?,?,?,?,?)",(int(eid),int(telegram_id),str(player_id),status,reason,self._now())); self.conn.commit()
    def get_training_participants(self,eid): return self.conn.execute("SELECT tp.*,p.nick FROM training_participants tp LEFT JOIN players p ON p.player_id=tp.player_id WHERE event_id=? ORDER BY created_at",(int(eid),)).fetchall()
    def create_report(self,chat_id,reporter_id,target_id,reason):
        cur=self.conn.execute("INSERT INTO reports(chat_id,reporter_id,target_id,reason,created_at) VALUES(?,?,?,?,?)",(int(chat_id),int(reporter_id),int(target_id),str(reason),self._now())); self.conn.commit(); return cur.lastrowid
    def get_reports(self,status='pending',limit=30): return self.conn.execute("SELECT * FROM reports WHERE status=? ORDER BY id DESC LIMIT ?",(status,int(limit))).fetchall()
    def add_moderation_log(self,chat_id,target_id,actor_id,action,reason='',duration=''):
        self.conn.execute("INSERT INTO moderation_logs(chat_id,target_id,actor_id,action,reason,duration,created_at) VALUES(?,?,?,?,?,?,?)",(int(chat_id),target_id,int(actor_id),str(action),str(reason or ''),str(duration or ''),self._now())); self.conn.commit()
